...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	if (vma->vm_file) {
		...
		if (susfs_should_apply_to_current() && SUSFS_IS_INODE_SUS_MAP(inode))
			return;
...
#ifdef CONFIG_KSU_SUSFS_SUS_MAP
	if (susfs_should_apply_to_current() &&
	    vma->vm_file && SUSFS_IS_INODE_SUS_MAP(file_inode(vma->vm_file)))
		return 0;
#endif
			    ...
			    size_t count, loff_t *ppos)
{
	...
	while (count && (start_vaddr < end_vaddr)) {
		...
		if (susfs_should_apply_to_current() &&
		    vma && vma->vm_file && SUSFS_IS_INODE_SUS_MAP(file_inode(vma->vm_file)))
			goto bypass_orig_flow;
#endif // #ifdef CONFIG_KSU_SUSFS_SUS_MAP
