...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (susfs_should_apply_to_current() &&
	    unlikely(dentry) && !IS_ERR(dentry) && dentry->d_inode &&
	    !found_sus_path && susfs_is_inode_sus_path(&nop_mnt_idmap, dentry->d_inode)) {
		if (d_in_lookup(dentry))
			d_lookup_done(dentry);
		if (!(flags & LOOKUP_RCU))
			dput(dentry);
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (susfs_should_apply_to_current() &&
		    is_nd_state_lookup_last_and_open_last && dentry && !IS_ERR(dentry) && dentry->d_inode &&
		    susfs_is_inode_sus_path(&nop_mnt_idmap, dentry->d_inode))
		{
			if (d_in_lookup(dentry))
				d_lookup_done(dentry);
